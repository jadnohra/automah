...
k_uni_log_and = [u'∧', '&']
k_uni_log_or = ['v']
k_uni_logical = k_uni_log_and + k_uni_log_or
k_uni_comp = u'∘'
k_uni_op = ['+','-','/','*','^',k_uni_comp] + k_uni_logical
...
def tok_print_group(group, tl, depth=0):
	for p in group['parts']:
		if (type(p) is not list):
			...
			print uni_pr(u'{}{} {}'.format(''.join([' ']*(depth)), '{}|_'.format(' ' if depth-1 else '') if depth else '', pp_indexed_toks(tl,p)))
...
def parse_refine_toks(toks, const_map):
    ...
    return ntoks
def parse_func_tokenize(func_str, parse_strict = True, const_map = {}):
    def is_float(s):
        try:
            float(s)
            return True
    ...
    if g_dbg:
        ...
        print uni_pr(pp_toks( tok_group_toks(gt[1], gt[0])))
...
expr_tree, expr_toks = parse_func_tokenize(sys.argv[1])
tok_print_group(expr_tree, expr_toks)
print expr_tree['parts']
print expr_toks
